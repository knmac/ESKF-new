...
void captureStop(int)
{
    if (g_Capture.recorder.isValid())
    {
		...
		char command[50];
		sprintf(command, "mv Captured.oni %llu.oni", millisecondsSinceEpoch);
		system(command);
    }
}
...
void captureSingleFrame(int)
{
	...
	if (colorFrame.isValid())
	{
		...
		unsigned long long timestamp = colorFrame.getTimestamp();
        timestamp = timestamp/1000;
        char fileName[50];
        sprintf(fileName, "data/%llu.jpg",timestamp);
