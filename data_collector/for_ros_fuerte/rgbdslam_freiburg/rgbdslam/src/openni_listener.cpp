...
#define OUTPUT_PATH "/home/cig/Desktop/RGBD/"
#define IMG_EXT "color.pgm"
#define DEP_EXT "depth.txt"
...
std::string OpenNIListener::GenerateID(const sensor_msgs::ImageConstPtr& depth_img_msg)
{
    int id1 = depth_img_msg->header.stamp.sec;
    std::stringstream strs1;
    strs1 << id1;
    std::string temp_str1 = strs1.str();

    int id2 = depth_img_msg->header.stamp.nsec;
    std::stringstream strs2;
    strs2 << id2;
    std::string temp_str2 = strs2.str();

    std::string ret_id = temp_str1 + "." + temp_str2;
                                      ...
                                      const sensor_msgs::CameraInfoConstPtr& cam_info_msg)
{
            ...
            || data_id_ % ps->get<int>("data_skip_step") != 0)
    {
        ...
        if(ps->get<bool>("use_gui")) //Show the image, even if not using it
        {
            ...
            cv::Mat visualgray;
            cvtColor(visual_img, visualgray, CV_BGR2GRAY);
            imwrite("/home/huixuan/Downloads/visul.pgm", visualgray, compression_params);
            imwrite("/home/huixuan/Downloads/depth.pgm", depth_mono8_img_, compression_params);*////////////////////////////////////////////////
            ...
            std::string temp_str = this->GenerateID(depth_img_msg);
            cv::Mat visualgray;
            cvtColor(visual_img, visualgray, CV_BGR2GRAY);
            imwrite(OUTPUT_PATH+temp_str+IMG_EXT, visualgray, compression_params);
            std::ofstream outFile;
            std::ostringstream oss;
            oss << OUTPUT_PATH << temp_str << DEP_EXT;
            ...
            for(int i=0; i<depth_float_img.rows; i++)
            {
                ...
                {
                    outFile << depth_float_img.at<float>(i,j)<<"\t";
                }
                outFile << endl;
            }
            outFile.close();
            ...
            if(addimage_)
            {
                cv::Mat visual_img_ = cv_bridge::toCvCopy(visual_img_msg, "bgr8")->image;
                Q_EMIT getImage(visual_img_, depth_float_img);///////////////////////////////////////////////////////////////////////////////////
            }
                                     ...
                                     const sensor_msgs::PointCloud2ConstPtr& point_cloud)
{
    ...
    {
        return;    //Visualization and nothing else
    }
...
void OpenNIListener::processNode(Node* new_node)
{
    ...
    compression_params.push_back(1);
    cv::Mat visualgray;
    cvtColor(visualization_img_, visualgray, CV_BGR2GRAY);
    imwrite("/home/cig/Desktop/RGBD/"+temp_str+"visul.pgm", visualgray, compression_params);
    imwrite("/home/cig/Desktop/RGBD/"+temp_str+"depth.pgm", visualization_depth_mono8_img_, compression_params);
    */
